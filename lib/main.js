...
function onAttach(worker) {
    ...
    worker.on('message', function(message) {
        if (message.name == 'settings') {
            ...
            var settings = JSON.parse(localStorage['settings'])
            var referer = settings['send_referer'] ? message.data.referer : ''
            ...
            }, { charset: message.data.charset, cookie: cookie, referer: referer })
        }
        else {
            console.log('else')
        }
...
function get(url, callback, opt) {
    ...
    if (opt.referer) {
        req.setRequestHeader('Referer', opt.referer)
    }
    req.send(null)
    return req
}
